...
$('.play-icon i').click(function() {
  var video = '<iframe allowfullscreen src="' + $(this).attr('data-video') +
              '"></iframe>';
...
$('#contact-submit').click(function(e) {
  ...
  // now when the validation is done we check if the error variable is false (no
  // errors)
  if (error == false) {
    // disable the submit button to avoid spamming
    // and change the button text to Sending...
    $('#contact-submit').attr({'disabled' : 'false', 'value' : 'Sending...'});
    ...
    $.post("sendmail.php", $("#contact-form").serialize(), function(result) {
      ...
      if (result == 'sent') {
        ...
        // re enable the submit button by removing attribute disabled and change
        // the text back to Send The Message
        $('#contact-submit')
            .removeAttr('disabled')
...
function initialize() {
  ...
  var style = [ {
    "stylers" : [
      {"hue" : "#ff61a6"}, {"visibility" : "on"}, {"invert_lightness" : true},
